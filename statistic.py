...
def file_name() -> list:
    ...
    if len(file_exsel) == 0:
        ...
        exit_error()
    elif len(file_exsel) > 1 and 'Статистика.xlsx' not in [str(i) for i in file_exsel]:
...
def read_file(files: list):
    ...
    result_df = user_oper(df)
    write_exsel(df=result_df, df_dost=df_dost, df_dost_s=df_dost_s, df_input=df)


def total_df_create(df, date_s, date_f):
    try:
        total_df_dict = dict()
...
def write_exsel(df=None, df_dost=None, df_dost_s=None, df_input=None):
    try:
        with pd.ExcelWriter('Статистика.xlsx', engine='xlsxwriter', datetime_format='DD/MM/YY HH:MM:SS') as writer:
            ...
            df_total = total_df_create(df, date_s=date_start, date_f=date_finish)
            ...
            for day in date_list_day:
                try:
                    ...
                    df_user_for_day = user_oper(df_temp)

                    df_total = total_df_create(df_user_for_day, date_s=day, date_f=day)
